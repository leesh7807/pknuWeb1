...
function mouseClickListener(centers, i) {
  ...
  return function () {
    ...
    if (centers[i]["lnmadr"] != "" && centers[i]["lnmadr"] != "null")
      center_callNumber.innerHTML += centers[i]["fshlcPhoneNumber"];
...
function infoWindowContent(centers, i) {
  ...
  if (
    ...
    centers[i]["fshlcPhoneNumber"] != "null"
  )
    contentNumbers += centers[i]["fshlcPhoneNumber"];
  var content =
    '<div class="wrap">' +
...
// 카테고리로 은행을 검색합니다
ps.categorySearch("PK6", placesSearchCB, map);
